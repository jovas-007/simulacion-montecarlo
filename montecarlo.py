...
class ScenarioProducer(threading.Thread):
    ...
    def __init__(
        self,
        scenarios: int,
        out_queue: "queue.Queue[Scenario | None]",
        seed: int = 4,
    ) -> None:
...
def main(simulations: int = 10_000, seed: int = 4) -> None:
    scenario_queue: "queue.Queue[Scenario | None]" = queue.Queue(maxsize=simulations // 10 or 1)

    producer = ScenarioProducer(simulations, scenario_queue, seed=seed)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Simulación Monte Carlo con arquitectura productor/consumidor")
    parser.add_argument(
        "-n",
        "--simulaciones",
        type=int,
        default=10_000,
        help="Número de escenarios a generar (predeterminado: 10 000)",
    )
    parser.add_argument(
        "-s",
        "--semilla",
        type=int,
        default=4,
        help="Semilla para el generador de números aleatorios (predeterminada: 4)",
    )
    args = parser.parse_args()
    main(simulations=args.simulaciones, seed=args.semilla)
